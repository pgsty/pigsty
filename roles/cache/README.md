...
RPM caches are plain repositories generated by `createrepo_c`. Legacy synthetic
module metadata is removed during every rebuild. On EL8/9, repositories that
intentionally replace packages from distribution module streams opt in with
`module_hotfixes` in their repository definition.
