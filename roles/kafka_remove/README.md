...
- When removing a strict subset, retire its KRaft voter and broker registration
  through a surviving member, including when the leaving host is unreachable
...
|-------------------|---------------|----------------------------------------------------------|
| `kafka_cluster`   | —             | Required cluster identifier                              |
| `kafka_seq`       | —             | Required numeric KRaft node ID                           |
...
| Tag                | Description                                 |
|--------------------|---------------------------------------------|
| `kafka-id`         | validate and derive removal identity        |
...
| `kafka_retire`     | retire voter and broker via a survivor      |
...
`kafka-rm.yml` skips hosts without `kafka_cluster`. A selected host must have a
valid cluster name and numeric `kafka_seq`; the role fails closed before any
removal step if either identity input is invalid.
